...
bool EquiRectWorldGenerator<_Grid>::load(IGridDescriptors *descriptors, const std::string &directory, LoadProgress &progress)
{
...
template<typename _Grid>
void EquiRectWorldGenerator<_Grid>::generateWorldOverview(LoadProgress &progress)
{
...
template<typename _Grid>
template<typename _FileIO>
bool EquiRectWorldGenerator<_Grid>::loadNormalize(const std::string &fileName)
{
    typedef generic::io::fs<_FileIO> fs;
...
template<typename _Grid>
template<typename _FileIO>
void EquiRectWorldGenerator<_Grid>::saveNormalize(const std::string &fileName)
{
    typedef generic::io::fs<_FileIO> fs;
...
template<typename _Grid>
void EquiRectWorldGenerator<_Grid>::generatePlates(LoadProgress &progress)
{
...
//Distance Map
    ...
    progress.update("Generating tectonic plates", 40, false);
//Border plate Map
...
template<typename _Grid>
void EquiRectWorldGenerator<_Grid>::generateContinents(LoadProgress &progress)
{
//    m_cellularNoise->SetFrequency(m_descriptorValues.m_continentFrequency);
//    m_cellularNoise->SetFractalLacunarity(m_descriptorValues.m_plateLacunarity);
//    m_cellularNoise->SetFractalOctaves(m_descriptorValues.m_plateOctaves);
